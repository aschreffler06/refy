...
export class CommandRegistrationService {
    ...
    async process(localCmds, args) {
        let remoteCmds = (await this.rest.get(Routes.applicationCommands(Config.client.id)));
        let localCmdsOnRemote = localCmds.filter(localCmd => remoteCmds.some(remoteCmd => remoteCmd.name === localCmd.name));
        let localCmdsOnly = localCmds.filter(localCmd => !remoteCmds.some(remoteCmd => remoteCmd.name === localCmd.name));
        let remoteCmdsOnly = remoteCmds.filter(remoteCmd => !localCmds.some(localCmd => localCmd.name === remoteCmd.name));
        switch (args[3]) {
            case 'view': {
                Logger.info(Logs.info.commandActionView
            ...
            case 'register': {
                if (localCmdsOnly.length > 0) {
                    Logger.info(Logs.info.commandActionCreating.replaceAll('{COMMAND_LIST}', this.formatCommandList(localCmdsOnly)));
                ...
                if (localCmdsOnRemote.length > 0) {
                    Logger.info(Logs.info.commandActionUpdating.replaceAll('{COMMAND_LIST}', this.formatCommandList(localCmdsOnRemote)));
            ...
            case 'rename': {
                ...
                Logger.info(Logs.info.commandActionRenaming
            ...
            case 'clear': {
                Logger.info(Logs.info.commandActionClearing.replaceAll('{COMMAND_LIST}', this.formatCommandList(remoteCmds)));
    ...
    formatCommandList(cmds) {
        return cmds.length > 0
            ? cmds.map((cmd) => `'${cmd.name}'`).join(', ')
            : 'N/A';
    }
}
//# sourceMappingURL=command-registration-service.js.map