...
export class OsuController {
    ...
    async getUser({ id = null, username = null }) {
        ...
        try {
            if (id) {
                ...
                user = await axios.get(`${this.osuEndpoint}/users/${username}/osu?key=username`, config);
            }
            else {
                throw new Error('No id or username provided');
            }
            return new OsuUserInfoDTO(user.data.id, user.data.username, user.data.statistics.global_rank, user.data.badges.length, Math.round(user.data.statistics.hit_accuracy * 100) / 100, user.data.statistics.level.current, user.data.statistics.play_count, Math.round((user.data.statistics.play_time / 3600) * 100) / 100, user.data.avatar_url);
        }
        catch (err) {
    ...
    async getBeatmapCombo(beatmapId) {
        ...
        const beatmap = await axios.get(`${this.osuEndpoint}/beatmaps/lookup?id=${beatmapId}`, config);
    ...
    async getBeatmapModdedDifficulty(beatmapId, mods) {
        ...
        const beatmap = await axios.post(`${this.osuEndpoint}/beatmaps/${beatmapId}/attributes`, { mods: mods }, config);
        return beatmap.data.attributes.star_rating;
    }
}
//# sourceMappingURL=osu-controller.js.map