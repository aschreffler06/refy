 ...
 */
export class AuctionSaleCommand {
    constructor() {
        this.names = [Lang.getRef('chatCommands.auctionSale', Language.Default)];
    ...
    async execute(intr, _data) {
        ...
        const playerEmbed = new EmbedBuilder()
            ...
            .addFields({ name: 'Seed', value: auctioned.seed.toString(), inline: true }, { name: 'Rank', value: auctioned.rank.toLocaleString(), inline: true }, { name: 'Current Bid', value: '$0', inline: true }, {
            name: 'Average Score',
            ...
            inline: true,
        }, { name: 'Description', value: auctioned.description.toString() }, { name: 'Ratings', value: `ETX: ${auctioned.etx} | SI: ${auctioned.skillIssue}` })
        ...
        bidCollector.on('collect', async (m) => {
            switch (true) {
                case /^bid [$]\d{1,2}(\.\d{1,2})?/.test(m.content): {
                    ...
                    if (!isBidValid(bid)) {
                        await InteractionUtils.send(intr, `<@${m.author.id}> Bid must be a multiple of $50`);
                        return;
                    }
                    ...
                    if (teamSize < 2 && currCash - bid < (2 - teamSize) * 50) {
                        await InteractionUtils.send(intr, `<@${m.author.id}> You cannot bid more than $${currCash - (3 - teamSize) * 50} to meet the required team size minimum`);
                        return;
                    }
                    if (bid > highestBid) {
                        if (highestBidder == m.author.id) {
                            await InteractionUtils.send(intr, `<@${m.author.id}> You are already the highest bidder`);
                            return;
                        }
                        if (currCash < bid) {
                        ...
                        await InteractionUtils.send(intr, `<@${m.author.id}> is now the highest bidder with a bid of $${bid}`);
                        embed.edit({
                            embeds: [
        ...
        bidCollector.on('end', async () => {
            if (highestBidder === null) {
                ...
                await InteractionUtils.send(intr, `Bidding ended! <@${highestBidder}> won with a bid of $${highestBid})}!`);
...
//# sourceMappingURL=auction-sale-command.js.map