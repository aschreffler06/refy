...
export class AuctionSaleCommand {
    ...
    async execute(intr, _data) {
        ...
        const playerEmbed = new EmbedBuilder()
            ...
            .addFields({ name: 'Rank', value: auctioned.rank.toString(), inline: true }, { name: 'Accuracy', value: auctioned.accuracy.toString(), inline: true }, { name: 'Badges', value: auctioned.badges.toString(), inline: true }, { name: 'Level', value: auctioned.level.toString(), inline: true }, { name: 'Play Count', value: auctioned.playCount.toString(), inline: true }, { name: 'Play Time', value: auctioned.playTime.toString(), inline: true })
        ...
        bidCollector.on('collect', async (m) => {
            switch (true) {
                case /^bid [$]\d{1,3}$/.test(m.content): {
                    ...
                    if (!isBidValid(bid)) {
                        await InteractionUtils.send(intr, `<@${m.author.id}> Bid must be a multiple of $25 and between $25 and $575`);
                        return;
                    }
                    if (bid > highestBid) {
                        highestBid = bid;
