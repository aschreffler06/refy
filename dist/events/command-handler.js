import { AutocompleteInteraction, ChatInputCommandInteraction, NewsChannel, TextChannel, ThreadChannel, } from 'discord.js';
import { RateLimiter } from 'discord.js-rate-limiter';
import { createRequire } from 'node:module';
import { CommandDeferType } from '../commands/index.js';
...
export class CommandHandler {
    constructor(commands, eventDataService) {
        this.commands = commands;
        this.eventDataService = eventDataService;
        this.rateLimiter = new RateLimiter(Config.rateLimiting.commands.amount, Config.rateLimiting.commands.interval * 1000);
    }
    async process(intr) {
        ...
        let commandParts = intr instanceof ChatInputCommandInteraction || intr instanceof AutocompleteInteraction
            ? [
                intr.commandName,
        ...
        if (!command) {
            Logger.error(Logs.error.commandNotFound
        ...
        if (intr instanceof AutocompleteInteraction) {
            if (!command.autocomplete) {
                Logger.error(Logs.error.autocompleteNotFound
                    .replaceAll('{INTERACTION_ID}', intr.id)
                    .replaceAll('{COMMAND_NAME}', commandName));
                return;
            }
            try {
                ...
                Logger.error(intr.channel instanceof TextChannel ||
                    ...
                    : Logs.error.autocompleteOther
                        ...
                        .replaceAll('{COMMAND_NAME}', commandName)
                        .replaceAll('{USER_TAG}', intr.user.tag)
                        .replaceAll('{USER_ID}', intr.user.id), error);
            }
            return;
        }
